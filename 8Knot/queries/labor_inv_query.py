...
def labor_inv_query(self, repos):
    """
    (Worker Query)
    ...
    SELECT DISTINCT cmt.repo_id, cntrb.cntrb_id, cntrb.cntrb_company, 
    COUNT(DISTINCT cmt.cmt_commit_hash) AS count,
    SUM(cmt.cmt_added + cmt.cmt_removed + cmt.cmt_whitespace) AS line_count
    ...
    GROUP BY cmt.repo_id, cntrb.cntrb_id, cntrb.cntrb_company
)
SELECT repo_id AS id, cntrb_company AS company, SUM(line_count) AS count
FROM cntrb_by_repo
WHERE repo_id IN (SELECT repo_id FROM repo_list)
GROUP BY repo_id, company
                """
